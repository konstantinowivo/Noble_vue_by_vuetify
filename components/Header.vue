<template>
    <!-- Drawer para navegación en pantallas pequeñas -->
    <v-navigation-drawer
        v-model="drawer"
        temporary
        left
        @click:close="drawer = false"
    >
        <!-- Drawer para pantallas pequeñas -->
        <v-list>
            <v-list-item to="/" @click.stop>
                <v-list-item-content>
                    <v-list-item-title>Inicio</v-list-item-title>
                </v-list-item-content>
            </v-list-item>
            <v-list-item to="/contacto" @click.stop>
                <v-list-item-content>
                    <v-list-item-title>Contacto</v-list-item-title>
                </v-list-item-content>
            </v-list-item>
        </v-list>
    </v-navigation-drawer>

    <!-- Barra de navegación -->
    <v-app-bar color="black" height="80">
        <!-- Icono de menú para abrir el drawer en pantallas pequeñas -->
        <v-app-bar-nav-icon @click="drawer = !drawer" class="d-md-none" color="yellow" />
        
        <!-- Botón de navegación sin overlay -->
        <v-btn class="nav-button" to="/" :overlay="false">
            <img src="../assets/title_nav.png" class="logo-image" alt="Logo" />
        </v-btn>

        <!-- Alinea los botones al extremo derecho -->
        <v-spacer></v-spacer>
        <div class="d-none d-md-flex">
            <v-btn class="nav-button" to="/" variant="text" :overlay="false" @click.stop>
                <span class="custom-text">Inicio</span>
            </v-btn>
            <v-btn class="nav-button" to="/contacto" variant="text" :overlay="false" @click.stop>
                <span class="custom-text">Contacto</span>
            </v-btn>
        </div>
    </v-app-bar>
</template>

<script>
export default {
    data() {
        return {
            drawer: false, 
        };
    },
};
</script>

<style scoped>
.custom-text {
    font-family: 'Bebas Neue', cursive;
    font-weight: bolder;
    font-style: italic;
    color: yellow;
    font-size: 150%;
}

.logo-image {
    max-width: 300px; 
    height: auto;
}

.nav-button {
    display: flex;
    align-items: center;
}

.v-app-bar-nav-icon {
    margin-right: 16px;
}

.nav-button:deep(.v-btn__overlay) {
    display: none;
}
</style>
